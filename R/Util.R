...
create_resultHeader <- function(traitType,
  ...
  if (traitType == "binary") {
    ...
    if (!isGroupTest) {
      ...
      if (IsOutputHetHomCountsinCaseCtrl) {
        resultHeader <- c(
          resultHeader, "homN_Allele2_cases",
  ...
  if (isGroupTest) {
    MACcateNumHeader <- paste0("Nmarker_MACCate_", seq(
      1,
...
writeOutputFileIndex <- function(OutputFileIndex,
                                 AnalysisType,
                                 nEachChunk,
                                 indexChunk,
                                 Start, # TRUE or FALSE, to indicate is the 'Output' is the first one to save into 'OutputFile'
